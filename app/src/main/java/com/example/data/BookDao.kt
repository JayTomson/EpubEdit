...
abstract class BookDao {
    ...
    @Transaction
    open suspend fun deleteChapters(chapters: List<Chapter>) {
        chapters.forEach { deleteChapter(it) }
    }

    @Transaction
    open suspend fun updateChaptersOrder(chapters: List<Chapter>) {
        chapters.forEach { updateChapter(it) }
    }

    @Transaction
    open suspend fun updateSourceFilesOrder(files: List<SourceFile>) {
